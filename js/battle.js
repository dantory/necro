 ...
 *  마지막에 체력바와 겹친다(V-20). */
/** ★★ V-23: 한 몸을 **여럿이 동시에** 때리면 숫자가 같은 자리에 같은 높이로 떠서
 *  두 글자가 겹쳐 한 덩어리가 됐다(첫 판 15초 그림의 「6」 두 개). 흩뜨리는 것이
 *  `vx` 하나뿐이었는데 ±8 세계단위(≈14px)라 **글자 폭보다 좁았고**, 게다가 무작위라
 *  같은 자리에 도로 겹칠 수도 있었다. 실측 — 그려진 숫자의 **41.5%가 제 넓이의 1/4
 *  이상**을 다른 숫자에게 먹혔다(`tools/v23_numgap.mjs`).
 *  ★ 먼저 **옆으로 흩뜨려** 봤고 접었다 — 줄(lane)을 나눠 ±14·±28 로 밀었더니
 *    「숫자끼리」는 나아지는데 **이웃의 체력바** 위에 올라앉아 V-20 이 되돌아왔다
 *    (바를 덮은 숫자 2.4% → 5.7%). 자리를 더 만드는 길은 화면이 이미 꽉 차 있다.
 *  ★ 그래서 **자리를 나누는 대신 숫자를 합친다** — 같은 몸에 이미 떠 있는 같은 종류의
 *    숫자가 있으면 새로 띄우지 않고 거기에 더한다. 개수가 3분의 1 줄어 겹칠 일이
 *    없어지고, 뭇매를 맞는 몸 위에서는 「6 6 6」 대신 「18」이 선다 — 이 장르는
...
export function popNum(x, y, v, kind, h = 0) {
  ...
  for (const o of S.nums) if (o.t > 0.2 && o.kind === kind
      && Math.abs(o.x - x) < NUM_MERGE_R && Math.abs(o.y - y) < NUM_MERGE_R
      && (!host || o.t > host.t)) host = o;
...
export function step(dt) {
  ...
  for (let i = S.mobs.length - 1; i >= 0; i--) {
    ...
    while (META.xp >= xpNeed(META.lv)) { META.xp -= xpNeed(META.lv); META.lv++;
      ...
      /* ★ V-114 — **갈 곳을 「어둠의 길」이 아니라 「스킬」이라 적는다.**
         「어둠의 길」은 그 창의 **제목**인데, 처음 켠 사람이 그 제목을 보려면 **이미 창을
         열었어야 한다.** 열기 전에 화면에 있는 이름은 아래 띠의 타일에 적힌 **「스킬」**
         하나뿐이다 — 이름이 둘이면 둘을 잇는 일이 사람 몫으로 남는다. V-113 이 편성 창에서
         가른 것과 같은 결이다([[carry-fixes-forward]]).
         ★ 하라는 말이 있는 결에서만 **「체력·마나 회복」을 뺀다.** 이 줄은 게임에서 가장
           긴 줄이고(1280 폭에서 오른끝이 메뉴 타일 왼끝을 9px 넘었다 —
           `tools/v114_logband.mjs`), 넘칠 때 묻히는 쪽은 늘 꼬리 곧 **하라는 말**이다.
           회복은 구슬 둘이 차오르며 이미 말한다. 나머지 두 결(배분·점수 없음)은 짧으니
           그대로 둔다. */
      say(`<b style="color:#ffff64">레벨 ${META.lv}</b> 달성`
        + (spent ? ` · 체력·마나 회복 · 스킬 점수 <b>${spent}</b> 배분`
          : left ? ` · 스킬 점수 <b>${left}</b> — <b>T</b> 「스킬」에서 찍는다`
            : " · 체력·마나 회복"));
