...
/* ══ 조사(助詞) ══ 이름 뒤에 붙는 「이/가」를 **받침으로 고른다.**
   ...
   때마다 · 제물을 바칠 때마다 · 주인이 지칠 때마다 나오는 줄이라 늘 보이던 흠이다.
   주인 넷 중 둘(짓쳐드는 파수꾼 · 저주받은 왕)이 받침을 가져서 **한쪽으로 굳힐 수도 없다.**
   ★ 이름은 태그로 감싸 오는 일이 많다(`<b>…</b>`) — 태그·꼬리 공백·닫는 괄호를 **떼고**
     마지막 «글자»를 본다. 숫자로 끝나도 한국어 읽기(영·일·이…)로 받침을 본다.
   ★ 「(으)로」는 ㄹ 받침이 예외다 — 「칼로」지 「칼으로」가 아니다. */
const JOSA_PAIR = { "이": ["이", "가"], "을": ["을", "를"], "은": ["은", "는"],
                    "과": ["과", "와"], "으로": ["으로", "로"] };
/** 숫자 한 자리를 한국어로 읽었을 때의 종성 코드(0 = 받침 없음, 8 = ㄹ). 영일이삼사오육칠팔구 */
const JOSA_DIGIT = [21, 8, 0, 16, 0, 0, 1, 8, 8, 0];
/** 받침 코드를 돌려준다. 한글·숫자만 안다 — 모르면 -1(받침 없음처럼 다룬다). */
export const jongOf = (word) => {
  const t = String(word == null ? "" : word)
    .replace(/<[^>]*>/g, "")                     /* 태그를 뗀다 */
    .replace(/[\s.,!?)\]}"'\u201d\u2019\u300d\u300f]+$/, "");   /* 꼬리 공백·닫는 부호를 뗀다 */
  if (!t) return -1;
  const c = t.charCodeAt(t.length - 1);
  if (c >= 0xac00 && c <= 0xd7a3) return (c - 0xac00) % 28;
  if (c >= 0x30 && c <= 0x39) return JOSA_DIGIT[c - 0x30];
  return -1;
};
/** 이름에 맞는 조사 **한 낱말**을 돌려준다 — `${lord.n}${josa(lord.n, "이")}` 처럼 쓴다. */
export const josa = (word, kind) => {
  const p = JOSA_PAIR[kind];
  if (!p) return "";
  const j = jongOf(word);
  if (kind === "으로") return (j <= 0 || j === 8) ? p[1] : p[0];   /* ㄹ 받침은 「로」 */
  return j <= 0 ? p[1] : p[0];
};
...
/* ══ 무리는 **한 놈이 아니다** ══ (2026-08-24 · V-7)
   ...
   W-2 가 배율을 키워 「붉은 덩어리」를 읽히게 만든 뒤에도, 읽히는 것이 **같은 놈 열넷**이면
   무리로 안 보인다. D2 의 타락자도 한 종이 아니라 **빛깔로 갈린 갈래**다(Carver·Devilkin·Dark One).

   ★ **그림을 새로 굽지 않는다** — 있는 스프라이트를 `ctx.filter` 로 물들인다. 시체에 쓴
     얹기(source-atop + 낮은 알파)와는 다른 길이다: 얹기는 채널을 **섞어** 명암을 뭉개는데,
     `hue-rotate` 는 밝기를 그대로 두고 **색상만** 돌려서 픽셀아트의 명암이 한 톨도 안 상한다
     (그래서 여기서는 알파를 낮출 까닭이 없다). 후보 여덟을 `tools/clan_sheet.mjs` 로
     한 장에 늘어놓고 눈으로 골랐다.
   ★ 고르지 않은 것과 그 까닭:
     · **보라(285°)** — 네크로멘서의 소환진·지배 표식이 쓰는 색이다. 적이 그 색을 입으면
       「내 것」과 섞인다(main.js 의 그 자리에 이미 같은 사고 기록이 있다).
     · **-30°** — 황토를 노렸는데 붉음이 **분홍**으로 갔다. 붉은 몸에서 아래쪽으로 도는 길은
       없다시피 하다.
     · **검은(brightness 0.55)** — 던전 바닥이 이미 어두워 통째로 묻힌다.
   ★ **수치는 한 톨도 안 건드린다**(D 계열 잠김) — 체력·피해·크기·발자국 전부 그대로다.
     크기 흔들기도 일부러 뺐다: W-2b 가 머리끝 여유를 `MOB_H` 로 증명해 뒀는데 그리는 쪽에서
     키를 흔들면 그 증명이 깨진다. */
export const MOB_CLAN = [
  { n: "",       f: null },                                              // 붉은 — 기준이 하나는 있어야 나머지가 「갈래」로 읽힌다
  { n: "푸른 ",   f: "hue-rotate(200deg)" },
  { n: "이끼빛 ", f: "hue-rotate(95deg) saturate(0.85) brightness(1.28)" },
];
/** 갈래는 **난수를 새로 들이지 않고** 고른다 — spawnMob 에서 `Math.random()` 을 한 번 더
 *  당기면 씨앗을 박은 검수기의 판이 통째로 갈려 여태 잰 수가 전부 못 쓰게 된다
 *  (그 자리 우두머리 판정에도 같은 주의가 적혀 있다). id 와 층을 섞어 흩는다 —
 *  둘러선 차례(id)가 곧 각도라, 단순히 `id % 3` 으로 하면 갈래가 **고리 무늬**로 규칙하게 선다. */
export const clanIdx = (id, f) => {
  let v = (Math.imul(id | 0, 0x9e3779b1) ^ Math.imul((f | 0) + 1, 0x85ebca6b)) >>> 0;
  v = Math.imul(v ^ (v >>> 15), 0x2545f491) >>> 0;
  return (v >>> 9) % MOB_CLAN.length;
};
export const clanOf = (m) => MOB_CLAN[(m && m.vr) | 0] || MOB_CLAN[0];
/** 갈래를 앞에 붙인 이름 — 「푸른 타락자 지배」처럼 로그에서도 갈린다. */
export const mobName = (m) => clanOf(m).n + (MOB_N[m && m.kind] || "시체");
...
const ALL_SKILLS = [
  ...
  /* ★ 글월에서 **시체 수를 뺐다**(V-131). 여기 적힌 `corpse:1` 은 「한 구라도 있으면
     쓸 수 있다」는 **문턱**이지 무는 양이 아니다 — 실제로 무는 것은 `novaGulp()`(16구)라,
     「시체 1 →」이 열여섯 배를 거짓말했다. 무는 양은 이제 벨트 칸이 `gulpOf()` 로
     적는다(battle.js). 값도 규칙도 안 바뀐다 — 적는 것만 참말이 된다. */
  { id:"nova",  n:"시체 폭발",    ico:"✹", mp:18, cd:2.2, corpse:1, d:"쌓인 시체를 한 입에 → 주위 광역 피해", dOld:"시체 1 → 주위 광역 피해" },
...
export const S = {
  ...
  shake: 0,
  /** ══ 다시 눕히기까지 남은 시간 ══ (V-43c) 층을 넘으면 앞 층 **그림만** 어둠에
   *  잠기는데(개수는 그대로), 지고 내려온 시체가 많으면 셈과 그림이 크게 갈린다
   *  (셈 138 대 그림 23). enterFloor 가 여기에 PILE_FADE 를 적어 두고, step 이
   *  다 잠긴 뒤에 모자란 만큼 새 층 바닥에 다시 흩는다. 0 이면 할 일 없음. */
  relayT: 0,
};
...
/* ★ `d` 는 **축의 이름만** 적는다 — 몫은 화면이 «지금 → 다음»으로 보인다(V-132).
 *  여태는 여기에 「+25」·「+8%」처럼 **한 단계당 몫**을 손으로 적어 두었는데, 셋 중 둘이
 *  거짓말이었다(실측 `tools/v132_upgrade.mjs`):
 *    · 생명력 「+25」 → 마을에서 실제로 오른 체력은 **16** (`hpMaxWith` 의 「몇 대 맞고
 *      버티나」 천장이 `bodyHp` 를 그대로 안 태운다 — 25 는 `bodyHp` 안의 몫이다)
 *    · 어둠의 힘 「+8%」 → Lv.30 에서 실제로는 **+3.4%** (0.08 은 `dmgMulOf` 의 **합** 안에
 *      드는 몫이라, 레벨·트리가 그 합을 키울수록 몫이 얇아진다 — %p 를 % 로 적었다)
 *    · 어둠의 힘은 **본인 피해도 같이** 올리는데(`dmgMulOf` 가 둘의 바탕이다) 「소환수」만 적었다
 *  값은 한 톨도 안 바뀐다 — 바뀌는 것은 **적는 것**뿐이다([[floor-erases-the-ramp]]).
 *  `dOld` 는 자를 보정하는 **문**이다(`__UPDOLD`). */
export const UPS = {
  hp:   { n:"생명력",   d:"최대 체력",        dOld:"최대 체력 +25",    base:14 },
  mp:   { n:"기력",     d:"최대 마나",        dOld:"최대 마나 +8",     base:16 },
  dmg:  { n:"어둠의 힘", d:"본인·소환수 피해", dOld:"소환수 피해 +8%",  base:22 },
  army: { n:"군세",     d:"소환수 상한",      dOld:"소환수 상한 +1",   base:40 },
};
