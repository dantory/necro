...
/* ══════════════════════════════════════════════════════════════
   ...
   ★★ **아래 목록은 2026-08-17 실측으로 넷이 깨졌다 — 「base 안전」을 더는 믿지 말 것.**
      base 12분(씨앗 1·3·9)이 지금은 **최고층 49·58·53 · 군세 31·36·28 · 시체 140 붙박이**다
      (`tmp/lhbase_*.json`). 즉 자동판이 과제를 깨고 유해를 벌고 있을 수 있고, 그렇다면
      「투자 0 인 회귀선」이라는 말 자체가 어긋난다. ROADMAP 「그 표를 재다가 둘을 봤다」 참고.
     · unique  — ✗ 깨짐. **D-4(2026-08-15)** 로 위로만인 유니크가 저절로 껴진다(autoWear).
                 base 12분에서 유니크 2·3·2 번 떨어진다.
     · army10  — ✗ 깨짐. 「armyCap 은 최대 6」이라 적었으나 실측 상한이 27·32·28 이다.
     · dig4    — 무덤 파기는 마을에서 금을 쓴다 → 자동판은 안 판다 (아직 성립)
     · rebirth — 자동판은 환생을 안 누른다 (아직 성립)
     · offer   — ✗ 흔들림. 「12분엔 한 번도 안 돈다」는 ⑥ 때 실측인데, 지금은 시체가
                 5~7분부터 상한(140)에 붙어 밸브 문턱(85%)을 넘는다.
     · feast   — 시체 잔치는 트리 끝 노드라 자동판엔 없다 (아직 성립)
     · gate    — ✗ 깨짐. 「26층에 못 닿는다」고 적었으나 지금 base 최고층은 49~58 이다.
