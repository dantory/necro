...
const fails = [], seen = {};
for (const [w, h] of [[1512, 863], [1400, 900], [1900, 900], [1900, 1000], [1100, 800]]) {
...
/* ③ 창을 넓혀도 배율이 안 커진다
   ★ **세로를 붙박아 놓고 가로만 견준다**(2026-08-24 · W-2b). 여태 1400×900 과
     1900×**1000** 을 견줬는데 그건 세로가 100px 다른 판이다 — 배율이 세로에도
     매달리게 되자(몸의 키를 세로 맞춤에 넣은 뒤) 그 100px 가 「폭 때문에 늘어났다」로
     둔갑했다. 재는 축이 둘이면 판정이 뒤집힌다([[threshold-and-ruler-must-match]]).
     같은 세로 900 에서 1400 → 1900 은 배율이 1.803 으로 **똑같다.** */
const a = seen["1400×900"], b = seen["1900×900"];
