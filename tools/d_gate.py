...
★ **D-15 로 두 팔의 «사람»이 바뀌었다**(2026-08-22 06:0x · core.js DIVE_DEF).
  기본값이 「처음부터」에서 「문이 열리면 저절로 제일 깊이」로 옮겨졌으므로
  · 「문 안 씀」은 이제 **일부러 «처음부터»를 고른 사람**이다(옛 기본값 · `__DIVE_DEF=0`).
  · 「문 씀」이 **게임 기본값**이다 — `tmp/collapse_def_d15` 가 `tmp/collapse_d13dive` 와
    열두 판 **바이트로 같다**(손잡이 없이 돌려 확인했다).
  자는 그대로 둔다: 고르는 사람도 진짜 사람이라 D 는 여전히 두 팔에서 서야 한다.
