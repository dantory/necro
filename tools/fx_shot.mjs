...
await ev2(`["fx/curse","fx/bonewall","fx/burnfx","fx/offerfx"].forEach(p=>window.sprite&&window.sprite(p))`);
await wait(1200);
for (const [id2, name] of [["amp","curse"], ["wall","wall"], ["burn","burn"], ["offer","offer"]]) {
  await ev2(`(async()=>{const B=await import("/js/battle.js"),C=await import("/js/core.js");
    ...
    B.cast("${id2}"); B.step(0.05); return 1;})()`, true);
  await wait(260);
