/* V-120 탐색 — **켜서 본다.** 오래 논 사람으로 아직 안 열어 본 창을 하나씩 연다
   (교리 · 건너뛰기 · 트리 · 가방 · 정산). 자가 아니라 «눈»이다([[play-it-before-measuring-it]]). */
...
const shot = async n => { const { data } = await S("Page.captureScreenshot", { format: "png" }); fs.writeFileSync("tmp/v120_" + n + ".png", Buffer.from(data, "base64")); console.log("  찍음 " + n); };
...
/* 정산 창은 LASTRUN 이 차 있어야 그린다 — 방금 34층에서 죽어 돌아온 사람으로 채운다. */
await ev(`(()=>{const R=window.__LASTRUN,C=globalThis.__C;if(!R)return 'no';
  R.has=true;R.dead=true;R.gold=48210;R.xp=91400;R.killed=417;R.floor=34;R.from=30;
  R.leveled=true;R.summoned=88;R.used=41;R.secs=724;
  const ks=["wand","robe","charm","helm","glove","ring"];
  R.loot=ks.map((k,i)=>C.mkItem(k,1+(i%3),i===2,30));
  return R.loot.length})()`);

for (const w of ["doctrine", "dive", "tree", "bag", "end"]) {
  const ok = await ev(`(()=>{if(!window.__openWin)return 'no';
    [...document.querySelectorAll('.win.on')].forEach(e=>e.classList.remove('on'));
    window.__openWin(${JSON.stringify(w)});
