...
for (const sk of ids) {
  ...
  const ate = await ev(`(()=>{const S=window.__S,B=globalThis.__B;
    ...
    if(mark)delete mark.boss; S.floor=f0; return ate})()`);
...
const fail = bad.corpse.length + bad.tag.length + bad.cost.length + bad.probe.length;
