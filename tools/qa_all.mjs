...
const RULERS = [
  ...
  /* V-99 — 툴팁 줄의 짝(이름 + 값)이 갈려 다른 줄에 앉는지. 문은
     `node tools/v99_wrap.mjs old` 로 부르면 운다(갈린 짝 2 · 보정 확인 완료). */
  { name: "v99_wrap",     args: [],                             secs: 240, tier: "fast", expect: /판정|갈린 짝/ },
  ...
  /* V-103 — 경험치 띠가 **「시체」의 눈금으로 읽히던 것**(왼끝이 픽셀 하나까지 같았다).
     차례만 바꿔 칸 줄 밑으로 내렸다 — 판 높이는 한 톨도 안 변한다(자가 함께 센다).
     `node tools/v103_xpbar.mjs old` 로 부르면 운다(사이에 낀 것 없음 · 보정 확인 완료).
     ★ 밑자락의 차례나 `.mid` 의 flex 를 만지면 이 자부터 돌린다. */
  { name: "v103_xpbar",   args: [],                             secs: 180, tier: "fast", expect: /통과|틀림/ },
  /* V-104 — **편성 창이 «못 세우는 몸»을 이름만 내걸고 침묵하는가.** 처음 켠 사람은
     해골밖에 못 세우는데(구울·골렘은 트리에서 찍어야 SKILLS 에 든다) 창은 넷을 다
     멀쩡한 칸으로 세우고 「벽(골렘) 1 · 몸(구울) 1」을 가장 밝은 값으로 적었다.
     열렸는지는 **벨트**에서 읽어(고칠 쪽과 재는 쪽을 가른다) 두 사람을 잰다 —
  ...
  /* V-106 — **좌판·대장간 칸 구석의 «0» 배지가 처음 켠 사람에게 가장 밝다**(열 + 넷).
     배지 빛깔이 등급색이라, 아무것도 안 가진 칸이 「가진 칸」으로 읽힌다 — V-102 가
     같은 창의 툴팁에 이미 박은 못을 격자로 옮긴 것이다. 값은 배지가 아니라 **심은
     세이브**(META.equip.tier · META.up)에서 읽고, 두 사람을 잰다 — 처음 켠 사람과
     갖춘 사람(과잉 수정 막이). 무덤 파기 칸(값 1)은 같은 창 안의 막이다.
     `node tools/v106_zeroq.mjs old` 로 부르면 운다(밝은 0 열넷 · 보정 확인 완료). */
  { name: "v106_zeroq",    args: [],                             secs: 180, tier: "fast", expect: /통과|미달/ },
  /* V-107 — **능력치 창의 강화 단추가 «값어치»를 «값»처럼 적는다**(「▲ 14」).
     값(`<b>56</b>`) 바로 오른쪽에 ▲ 를 달고 서 있어 「14 오른다」로 읽히는데, 14 는
     금값이고 오르는 몫은 25% 다. 게임의 다른 값어치는 어디서나 「N 금」이다.
     값은 단추가 아니라 **자가 제 손으로 다시 셈해**(META.up + UPS 밑값) 견주고,
     좁은 자리에서 두 판으로 서는 것까지 **판마다** 오른끝을 맞춘다.
     `node tools/v107_upcost.mjs old` 로 부르면 운다(금 없는 단추 24 · 보정 확인 완료). */
  { name: "v107_upcost",   args: [],                             secs: 240, tier: "fast", expect: /통과|미달/ },
  /* V-108 — **처음 켠 사람에게 「가장 깊이 1층」이라고 말한다.** `META.deepest` 밑값이 1 이라,
     던전 문 앞에 서서 한 걸음도 안 뗀 사람의 화면 **세 자리**(마을 머리글 · 「어디부터」 창 ·
     초기화 창)가 「1층까지 내려가 봤다」로 읽힌다. 규칙은 안 건드리고 **글월만** 가른다.
     막이가 셋이다 — 특히 **1층에서 죽고 온 사람**(runs 1)에게는 그 말이 맞으므로 그대로여야 한다.
     `node tools/v108_deep.mjs old` 로 부르면 운다(거짓 12 · 보정 확인 완료). */
  { name: "v108_deep",     args: [],                             secs: 300, tier: "fast", expect: /통과|미달/ },
  /* V-109 — **위 띠의 글자가 밝은 구역 바닥 위에서 지워진다.** `#top` 이 깐 판이
     `#000000d8 → 투명` 한 줄이라 **글자가 서는 높이에서 이미 반쯤 사라져** 있고, 그 뒤는
     층마다 색이 갈리는 무대다. 뼈의 회랑(9층)·마른 피의 골(40층)에서 「남은 적 N」이
     **1.93:1** 까지 내려갔다 — 「층」이 5.15 인 것과 나란히 두면 진행을 말하는 유일한 수가
     화면에서 가장 안 읽히는 글자였다.
     자는 **끄고·켜고·다시 끄고 세 장**을 찍어 두 바탕이 같았던 자리에서만 글자를 가려내고,
     견주는 대상은 **켠 화면에서 글자 바로 둘레**다(그늘은 글자와 함께 뜨므로 지운 장을
     바탕으로 쓰면 그늘이 통째로 안 세어진다). 낱말은 통째로 읽히므로 **세로 8px 토막마다**
     따로 견주고 가장 나쁜 토막을 그 라벨의 값으로 삼는다.
     `node tools/v109_head.mjs old` 로 부르면 운다(3 미만 일곱 · 보정 확인 완료). */
  { name: "v109_head",     args: [],                             secs: 300, tier: "fast", expect: /통과|미달/ },
  /* ★ V-110 — **정산의 「내려간 깊이」가 간 적 없는 길을 그린다.** 건너뛰기가 열린 뒤로
     판이 10층·23층에서 시작하는데, 거기서 한 층도 못 내려가고 끝나면 `10→10층` 이라
     적었다 — 화살표는 「여기서 저기로」인데 간 데가 없다. 처음 판이 8초 만에 끝나는
     자리(V-14b)라 자주 보이는 거짓말이다. 자는 LASTRUN 을 **손수 심어** 다섯 판을
     그리고, 화면에 적힌 층수를 심은 값과 견준다(화살표가 옳게 서야 하는 10→23층이
     과잉 수정 막이다). 창이 세로로 넘치는 것(1280×620 · 38px)은 V-100b 라 문 밖이되
     **더 나빠지면** 운다 — 딱지가 길어졌으므로.
     `node tools/v110_depth.mjs old` 로 부르면 운다(거짓 12 · 보정 확인 완료). */
  { name: "v110_depth",    args: [],                             secs: 240, tier: "fast", expect: /통과|미달/ },
  /* ★ V-111 — **능력치 창의 「×1.00」이 진짜 값과 똑같이 밝다.** 처음 켠 사람의 여섯 줄
     가운데 둘(본인 피해 · 소환수 피해)이 마을에서 늘 ×1.00 인데, 빛깔이 「체력 56」과
     같아 그 옆의 진짜 값까지 안 읽힌다. 같은 창이 유해·깊이·금 획득 셋은 이미 걸러 두고
     **피해 두 줄에만 안 옮겼다**([[carry-fixes-forward]]). 줄도 단추도 안 지우고
     빛깔만 눕힌다(V-106 배지 0 과 같은 `.off`). 자는 두 사람을 재고(처음 켠 사람 ·
     강화 3·Lv.5 로 배수가 붙은 사람 = 과잉 수정 막이), 「중립인가」를 **화면 글자가 아니라
     `__STATTRUTH`(core.js 함수)** 로 가른다.
     `node tools/v111_neutral.mjs old` 로 부르면 운다(뜻 없는데 밝다 2 · 보정 확인 완료). */
  { name: "v111_neutral",  args: [],                             secs: 180, tier: "fast", expect: /통과|미달/ },
  /* ★ V-112 — **대장간 설명칸의 「재련」 줄이 처음 켠 사람에게 열 자리 전부 `+0`** 이다.
     창 안에서 가장 긴 줄(두 줄로 접힌다)이 가장 뜻이 없었다. 오른 자리만 적고 하나도
     없으면 줄째 접는다 — 「재련이 있다」는 바로 아래 줄(「다음 재련 N 금 — 저절로 산다」)이
     그대로 말해 주므로 사는 길이 안 사라진다(그래서 V-111 처럼 가라앉히지 않고 접었다).
     자는 세 사람을 재고(처음 켠 사람 · 셋만 오른 사람 · 열 자리 다 오른 사람 = 과잉 수정
  ...
  /* V-123 — 트리 툴팁이 **지금 얼마인가**를 말하는가. 수치를 건드리는 칸(해금 칸 여섯 제외)은
  ...
  /* V-128 — **빈손으로 돌아온 사람의 정산 창이 낮은 화면에서 세로로 넘치던 것**(V-100b).
     V-100 의 자는 좌판 «밀림»만 판정하고 넘침은 「곁들여 본 것」으로만 적어서, 자가 있어도
     아무도 안 울었다 — 한 물음에 한 자라 따로 세웠다. 화면 넷(1280×620·1152×648·
     1366×700·1512×863)을 **같은 판**으로 잰다. 문은 `body.v100bold` 이고, 자 안에
