...
for k in 띠들:
    ...
    ps = Z["피해"] / Z["초"]
    # ★ 0 대는 얇은 표본이 아니라 **그 자체가 답**이다(50-99층 3018초 무피해를 깔고 뭉갤 뻔했다).
    믿 = ("" if (Z["초"] >= 60 and (Z["맞은횟수"] >= 20 or Z["피해"] == 0))
          else "  ⚠표본얇음")
    if Z["초"] >= 60 and Z["피해"] == 0: 믿 = "  ★한 대도 안 맞았다"
    print(f"  {k:>6}층 · 머문 {Z['초']:.0f}초 · 초당 피해 {ps:.2f} ({Z['맞은횟수']}대) · "
          f"절반아래 {Z['절반아래']:.0f}초 · {원}{믿}")
    if k == "1-9": 얕 = ps

print("\n판정:")
깊 = [k for k in 띠들[1:] if 합[k]["초"] >= 60 and (합[k]["맞은횟수"] >= 20 or 합[k]["피해"] == 0)]
