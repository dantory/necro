/* ★★ V-194 폐기 고지 — 이 자의 ㉠ CV 통과선(≥0.35)은 폐기됐다. ㉡ 축정렬만 유효하다.
 *   까닭: 0.35/0.52 는 「점」이 겹칠 수 있는 2D 포아송 기준인데, 충돌 반지름을 가진 물체는
 *   서로 겹치지 못해 최근접 거리가 r합 근처에 몰린다 → CV 가 «구조적으로» 낮다(0.29 는 결함이
 *   아니라 분리력 있는 무리의 정상값). 즉 도달 불가한 수였다. 격자 여부를 가르는 자는 ㉡ 축정렬이고,
 *   그건 처음부터 0.33(무작위 기댓값)으로 통과였다. 이 자를 다시 들면 ㉡ 만 보라. [[retired-ruler-cannot-raise-alarm]]
 *
