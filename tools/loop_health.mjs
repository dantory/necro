...
await S("Page.addScriptToEvaluateOnNewDocument", { source:
  `Math.random = (() => { let s = (${SEED} >>> 0) || 1;
   ...
   ${process.env.LH_ADDDMG != null ? `globalThis.__ADD_DMG = ${+process.env.LH_ADDDMG};` : ""}
   ...
   ${process.env.LH_HPGROW != null ? `globalThis.__HPGROW = ${+process.env.LH_HPGROW};` : ""}
   ${process.env.LH_FLOORP != null ? `globalThis.__FLOOR_P = ${+process.env.LH_FLOORP};` : ""}
    ...
    try {
      /* 이 0.05 초를 어느 통에 부을지 — **step 전의 판**이 그 동안의 상태다. */
      {
        ...
        /* ── ★ **눈금을 사다리로 낸다** (08-17 08:0x) ────────────────────────────
           ...
           평균 30 구인 못은 그 문턱에 영영 안 닿는다 — 바닥이 문턱에서 멀면 그 수는
           눈금이 아니라 상수다([[floor-far-from-threshold]]). 그래서 문턱을 하나 새로
           **고르지 않고** 못의 얕은 쪽 분포를 통째로 찍는다: 5·10·20·30·40·60 구 아래에
           머문 초. 사다리를 보고 **어느 칸이 팔에 따라 갈리는지** 를 눈으로 고른다.
           ★ 첫 1분은 빼고 센다 — 판이 열린 직후는 어느 팔이든 못이 비어 있다. */
        if (R.t >= 60) {
          G.늦초 += 0.05; G.못합 += S.corpses;
          if (S.corpses < G.못바닥) G.못바닥 = S.corpses;
          const 칸 = [5, 10, 20, 30, 40, 60];
           ...
           ★ 읽기만 한다 — 게임 상태를 안 건드리므로 난수 소비가 안 달라진다(A/B 유지). */
        {
          ...
          /* ★ **D-3 의 자** — 「우두머리의 몸이 화면에 남는가」. 위 우두머리초 는 띠(10-24)
             단위 합이라 D-3 의 끝 조건(12~19층)과 분모가 다르다 — 조건을 쓴 그 창을
             그대로 센다([[threshold-and-ruler-must-match]]).
             표본 = 12~19층에 있던 0.05초 · 잡힘 = 그중 살아 있는 우두머리가 하나라도 있던 것 ·
             보임 = 그중 다 서 있던(born 끝난) 것. 고리를 그리는 조건이 「champ && !born」 이라
             둘을 갈라 센다. 읽기만 하므로 난수 소비가 안 달라진다. */
          if (f >= 12 && f <= 19) {
            const D3 = V.우두머리봄 || (V.우두머리봄 = { 표본: 0, 잡힘: 0, 보임: 0 });
            D3.표본++;
            let 산 = 0, 선 = 0;
            for (const m of S.mobs) if (m.champ) { 산 = 1; if (!(m.born > 0)) 선 = 1; }
            if (산) D3.잡힘++;
            if (선) D3.보임++;
          }
        }
...
if (시간) {
  ...
  if (G && G.초 > 0) {
    ...
    console.log(`→ 옛 끝조건(참고): 시체없음 **${Math.round(G.시체없음 / G.초 * 100)}%** ` +
                `— ${G.시체없음 / G.초 >= 0.15 ? "닿았다" : "아직"}` +
                (G.빈늦 < 1 && G.시체없음 > 0
                  ? ` ★ 그 초는 **판이 열린 첫 1분 안**뿐이다 — 자원이 모자란 것이 아니라 판이 시작한 흔적이다`
                  : ""));
    /* ── ★ **못의 얕은 쪽 사다리** ── 문턱을 미리 고르지 않고 분포를 통째로 찍는다.
       팔에 따라 갈리는 칸이 눈금이 될 자격이 있다(안 갈리는 칸은 상수다). */
    if (G.늦초 > 0) {
      const 칸 = [5, 10, 20, 30, 40, 60];
      const 사다리 = 칸.map((c, j) => `<${c}구 ${Math.round(G.얕은[j] / G.늦초 * 100)}%`).join(" · ");
      console.log(`  못(첫 1분 뺀 ${G.늦초.toFixed(0)}초) — 평균 ${(G.못합 / (G.늦초 / 0.05)).toFixed(1)}구 · ` +
  ...
  if (V && V.초 > 0) {
    ...
    /* D-3 — 우두머리의 «몸»이 12~19층 화면에 얼마나 남는가(끝 조건 30%). */
    { const D3 = V.우두머리봄;
      if (D3 && D3.표본 > 0)
        console.log(`우두머리 몸 — 12~19층 표본 ${D3.표본} · 살아 있음 **${(D3.잡힘/D3.표본*100).toFixed(1)}%** · ` +
                    `다 서 있음(고리 그려짐) ${(D3.보임/D3.표본*100).toFixed(1)}%`); }
