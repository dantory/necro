  ...
  for (let i = 0; i < n; i++) {
    try {
      /* 이 0.05 초를 어느 통에 부을지 — **step 전의 판**이 그 동안의 상태다. */
      {
        ...
        const Z = V.띠[띠] || (V.띠[띠] = { 초: 0, 피해: 0, 맞은횟수: 0, 절반아래: 0, 원인: {} });
        Z.초 += 0.05; R.띠 = 띠;
  ...
  if (V && V.초 > 0) {
    ...
    for (const k of 띠s) {
      ...
      console.log(`  ${k.padStart(6)}층 · 머문 ${Z.초.toFixed(0)}초 · 받은 피해 ${Math.round(Z.피해)} ` +
                  `(초당 ${(Z.피해 / Z.초).toFixed(1)} · ${Z.맞은횟수}번) · 절반아래 ${Z.절반아래.toFixed(0)}초 · ${원}`);
    }
    ...
    if (넉넉(얕) && 깊) {
      const a = 얕.피해 / 얕.초, b = 깊.피해 / 깊.초;
      console.log(b < a * 0.5
        ? `→ 깊은 띠(${깊띠})에서 **닿는 피해가 초당 ${b.toFixed(2)} 로 얕은 띠(${a.toFixed(2)})의 ` +
          `${Math.round(b / Math.max(1e-9, a) * 100)}%** 다 — 소환수 체력을 비틀어도 위험은 안 생긴다(구조를 고칠 것)`
        : `→ 깊은 띠에서도 피해는 초당 ${b.toFixed(2)}(얕은 띠 ${a.toFixed(2)})로 닿고 있다 — 못 죽는 이유는 **체력 쪽**이다`);
    }
    if (V.절반아래 < V.초 * 0.02)
