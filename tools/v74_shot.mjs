/* V-74 전/후 한 장 — 위가 「전」(일지가 인물 밑), 아래가 「후」(일지가 수치 아래 · 인물 옆).
     node tools/v74_shot.mjs [폭 높이]   → tmp/v74_cmp.png */
...
const shot = async (old, out) => {
  await ev(`window.__NOJSIDE=${old}; window.__closeAll&&window.__closeAll(); window.__openWin("stat");`);
  await wait(800);
  const s = await S("Page.captureScreenshot", { format: "png" });
  fs.writeFileSync(out, Buffer.from(s.data, "base64")); console.log("wrote", out);
};
await shot(true,  "tmp/v74_before.png");
await shot(false, "tmp/v74_after.png");
