...
for (const DOC of DOCS) {
  ...
  for (const SEED of SEEDS) {
    ...
    /* ★★ D-54 · **위험을 센다 — 판을 한 톨도 안 건드리고 이미 찍어 둔 hist 만 되읽는다.**
       ...
       ★★ D-56 · **사건 수는 버렸다 — «있던 초»로 다시 센다.** D-55 에서 「무너짐 회수」의
       A/A 바닥(5)이 A/B 신호(4)보다 크고 부호까지 뒤집혔다. 한 자릿수 사건 수는 눈금이
       아니다. 대신 **군세가 문턱 아래에 «있던 초»를 판 길이로 나눈 몫**을 낸다 —
       같은 여섯 판에서 「군세바닥·군세0초」 같은 이어진 값은 안 흔들렸으므로.
        · `낮은몫3`·`낮은몫5` = (군세 ≤3 / ≤5 였던 초) ÷ 던전에서 산 채로 있던 초
       분모는 **판 길이(ff)가 아니라 던전·산 채 표본**이다 — 마을·죽음 구간은 분자에서도
       빼므로 분모에 두면 몫이 판마다 다른 뜻이 된다([[threshold-and-ruler-must-match]]).
    ...
    const 위험 = (() => {
      ...
      /* ★ D-56 · 표본이 없으면 몫을 null 로 남긴다 — 0% 로 찍으면 「위험 없음」으로 읽힌다
         ([[silent-zero-is-not-an-observation]]). */
      const 몫 = n => (셈 ? +(n / 셈 * 100).toFixed(1) : null);
    ...
    { const W = o.위험;
      if (!W) console.log(`    (위험 장부 없음 — hist 가 비었다)`);
        ...
        /* ★ D-56 · 사건 수 «아래»에 이어진 값을 나란히 찍는다 — 두 자를 같은 판에서 견주려면
