...
if (!L?.n || !E?.n) { console.log("── 판정 ── 한쪽을 못 읽었다."); }
  ...
  const ok2 = 층비 >= 0.9 && 죽음차 <= 1.0;
  console.log(`② 정예가 못 살지 않는가 — 최고층 ${(층비 * 100).toFixed(0)}% (${m(E.최고층).toFixed(0)} 대 ${m(L.최고층).toFixed(0)}) · ` +
    `죽음 ${죽음차 >= 0 ? "+" : ""}${죽음차.toFixed(1)} → ` + (ok2 ? "**갈래로 선다**" : "**덫이다** — 고르면 손해다"));
