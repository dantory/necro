...
#                        ★ 그 base 합 160 은 **유해 7구(×1.56)를 포함한 값**이다 — 자동판이
#                          과제 넷을 4~7분에 깨고 뒤 절반을 부푼 채 달린다(js/core.js ⑦ 주석).
#                          XP 를 늦추는 팔은 그 과제도 늦게 깨서 **배수까지 같이 늦는다** —
#                          팔 사이 차이의 일부는 XP 곡선이 아니라 유해 되먹임이다.
