/* V-71 「켜서 본다」 — 「능력치」 창의 인물을 옛 꼴(전)·고친 꼴(후)로 한 판에서 찍는다.
   자가 통과해도 사람 눈으로 한 번 본다([[play-it-before-measuring-it]]).
   node tools/v71_shot.mjs   → tmp/v71_before.png · tmp/v71_after.png */
...
for (const [old, out] of [[1, "tmp/v71_before.png"], [0, "tmp/v71_after.png"]]) {
  await S("Page.reload", { ignoreCache: true }); await wait(1600);
  if (old) await ev(`window.__NOPDMID = 1`);
