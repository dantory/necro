...
## ▶ 지금 도는 판: V-273 — 08:00 감시가 컷을 열어 본 흠 다섯 + V-272 화면 판정 (띄움 2026-09-03 08:03)
브리프 `tmp/v273_brief.md` · 로그 `tmp/necro_hs_v273.log`(`.done`) · 상한 2700초(→ 08:48 께).
로그 첫 줄 확인함(LOG/CMD 정상 · 잠금 에러 아님 · 겹친 opencode 없음).
- **① `__MAPLEGEND3`** — 전체지도에 범례에 없는 표식이 아직 둘(흰 마름모=나 · 흰 삼각=계단).
- **② `__TRAPHUE`** — 함정 결이 노랑이라 «물건 빛무리·화로 고리»와 같은 색. 위험색으로 갈래별 분리.
- **③ `__ITEMLABEL`** — 바닥 물건 이름표 두 줄이 겹쳐 상자 그림을 통째로 가림. 밀어내기+지시선.
- **④ `__CAMROOM`** — 컷 여덟 중 여섯에서 화면 40~50%가 방 밖 벽. 카메라를 방 쪽으로 치우침.
- **⑤ V-272 화면 판정** — `a244303` 이 rot/bone 타일을 갈았는데 화면으로는 아무도 안 봤다. 방마다 바닥 밝기가 갈리는 것도 같이 본다.
- **⑥ `__TRAPFALL`**(남으면) — 바닥 꺼짐: 다음 층으로 떨어지는 다섯째 갈래.

### 08:00 감시의 V-271 판정 (컷 여덟 직접 열어 봄 · 2~3배 확대)
**통과.** `aura_fix` 에서 발밑 초록 빛무리+어깨 초록 마름모 룬 셋이 또렷(V-270 때 0픽셀이던 것) ·
`traproom_door` 문턱에서 방 안 함정 결 여럿 · `flame_burst` 주황 도트 · `map_title` 범례에
함정 방/안 치운 방/치운 방/적 들어가고 상인 하늘빛. 콘솔 오류 0.
**다만 흠 다섯**(위 ①~⑤).
