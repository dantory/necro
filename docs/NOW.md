...
## ▶ 지금 도는 판: V-267 — 세트 장비(짝을 맞추면 판이 달라진다) · 03:00 감시 흠 셋 (03:30 띄움 · 검증 전)
브리프 `tmp/v267_prompt.md` · 로그 `tmp/necro_hs_v267.log`(.done 이 마커).
- **① 세트 장비 `__SETGEAR`** — `grep -c "세트|SET_|setBonus" hs/loot.js hs/main.js` = **0·0** 이라 이 축이 통째로 없었다.
  PLAN 의 큰 항목(①환생=V-239 · ③군세 갈래=V-240/244/245 · ④주인 수법=bossKind · ⑤유니크 · ⑦일지)은 이미 들어가 있음을 grep 으로 확인함.
  세트 셋×3점 · 2점/3점 짝 보너스(3점은 규칙이 바뀌는 것) · 세트색 툴팁(가진 것 밝게·없는 것 어둡게).
- **②** 03:00 감시가 적은 흠 셋 ㉠ 지도 복도/방 대비 ㉡ 시체 바닥 그림자 ㉢ 숫자 뒤 조사 띄어쓰기.


### 03:00 감시가 컷을 열어 보고 적은 흠 셋(다음 판 후보 · V-266 자체는 통과)
- **㉠ 지도에서 복도와 방이 여전히 한 덩이로 읽힌다.** ③ 이 복도를 한 단계 어둡게 했는데
  `map_on` 을 열어 보면 복도 폭이 넓고 색차가 얕아 방과 방이 «붙은 큰 방»처럼 보이는 자리가 있다
  (왼쪽 무리·가운데 오른쪽). 색을 더 내리거나 복도 폭을 지도에서만 가늘게 그릴 것.
- **㉡ 시체가 «바닥에 놓인 것»으로 안 보인다.** `corpse` 컷의 해골 표식은 그림자가 없고
  후광 원이 UI 아이콘처럼 떠 있다. 바닥 그림자 한 겹 + 살짝 눕힌 몸이면 판 위의 물건이 된다.
- **㉢ 죽음 판 글월 띄어쓰기** — 「금 1.5천 을 B7층에 두고 왔다」 → 「금 1.5천을」.
