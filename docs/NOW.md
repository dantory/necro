...
## ▶ 지금 도는 판: V-264 — 지도를 «한눈에» · 마지막 색 튐 둘 (착수 2026-09-03 00:33)
D2 문법 다섯은 V-261~263 으로 다 닫혔다. 남은 것은 **길을 찾는 눈**이다.
① 전체 지도 `__BIGMAP`(Tab, 미니맵과 한 근원·배율만 다름·게임 안 멈춤) ② 표식을 꼴로 가름 `__MAPICONS`
(보물방 ◆금 · 소굴 ▲붉음 · 제단 ✚ · 상인 ■청록 · 계단 ▼흰 · 보통 상자는 작은 점으로 낮춤 — 지금 보물방 #ffcf5a 가 상자 #ffd24a 와 같은 노랑·같은 꼴)
③ 남은 색 튐 둘 `__MOBTINT2`(초록 몹 라임→썩은 황록 · 자주 로브 몹은 sepia 를 먼저 태워 원색 지움).
지켜야 하는 것: 그리기·UI 전용 → **genFloor 지문 ON=OFF byte-동일** · `hs_v207_walk` 벽밖 0% · `hs_v219_foeshot` p95 ≤16.7ms(지도 연 채로도).
★ **컷 굽기 전 모달(「돌아왔다」·서약)을 먼저 닫는다** — V-262·V-263 두 판 연속으로 컷 한복판이 덮였다.
브리프 `tmp/hs_v264_prompt.md` · 로그 `~/.openclaw/workspace/tmp/necro_hs_v264.log` · 컷 `tmp/hs_v264_{bigmap,bigmap_deep,icons_{on,off},tint_{on,off}}.png`
