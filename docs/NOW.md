...
## ✔ 끝난 판: V-287 — 18:43 병수님 「D2 같은 느낌이 안 난다·방향을 못 잡네」 → ① UI 를 D2 결로(좌우 구슬·아래 가운데 벨트) (2026-09-03)
docs/D2_FEEL.md 가 V-260~286 스물일곱 판의 잔손질을 접고 «큰 것 셋»(①UI D2결·②톤 어둡게·③아이콘 픽셀랩)을 시켰다. 브리프 「다 못 하면 하나라도 끝까지」에 따라 **가장 크고 시점을 안 바꾸는 ① UI 만 끝까지** 했다(②③ 은 안 함·아래 명시). 만진 곳 `hs/main.js`(HUD 갱신+플래그)·`hs/index.html`(구슬 요소 둘)·`hs/hud.css`(d2ui 레이아웃) **셋뿐** — map.js/genFloor 무접촉·새 자 파일 없음(일회용 베이커 `tmp/hs_v287_ui.mjs`).
- **① `__UI`(=body.d2ui)** — UI 가 히어로시즈(좌상단 가로 바 + 좌하단 벨트 + 우상단 지역, 사방에 흩어짐)였던 것을 **D2 결로 한 덩어리로** 모았다:
  - **좌우 큰 구슬** — 왼 체력(붉음)·오른 마나(푸름)를 «액체가 담긴 꼴»로: fill 높이=hp%/mana%·유리 하이라이트(radial)·표면 반달 띠가 느리게 흔들림(transform keyframe·GPU). **바가 아니라 구슬** = D2_FEEL 의 「D2 를 D2 로 보이게 하는 가장 센 표시」. 좌상단 가로 바(#tl)는 d2ui 에서 숨기고 구슬이 대신한다.
  - **아래 가운데 한 덩어리** — 경험치 띠·금·스킬 칸(Q E R V M)·물약 칸(1~4 U T C)을 beltwrap 세로로 쌓아 화면 아래 가운데로 모음(#bl 을 left:50%·translateX). 지역·미니맵은 우상단 그대로(D2 도 미니맵을 그 자리에 둔다). 조작 안내(#hint)는 d2ui 에서 숨김(H 로 전체 안내 있음·D2 는 상시 안내 없음·기존에 위로 올렸더니 적 이름표와 겹쳐 숨김으로 정정).
  - **틀은 CSS 로 «한 번» 그려지고** 프레임마다는 액체 높이·글자 넉 줄만 갱신(hud p95 0.5ms) → 브리프 「렉을 늘리지 말 것」 지킴. `__UI=false` → 옛 흩어진 UI(컷 `hs_v287_ui_off.png` 로 확인).
- 컷 **직접 열어 판정**: `tmp/hs_v287_ui_{off,on}.png`·`tmp/hs_v287_ui.png`(=on). off 는 좌상단 가로 바+좌하단 벨트(히어로시즈)·on 은 **좌우 구슬 + 가운데 벨트** → 「**D2 로 보이기 시작한다**」(docs/ref/d2_catacombs.png 의 좌우 구슬 결에 가까워짐). ★ `tmp/rep_cmp_d2.png` 는 리포에 없어서(브리프가 가리켰으나 파일 부재) d2_catacombs.png 와 견줬다.
- 되돌림/회귀: 셋 다 DOM/CSS(genFloor 무접촉) → `tmp/_v276_fp.mjs` F1/3/5/10/30 **byte-동일**(4341720539·cebe184b88·07968a97a3·e9e3aa0cbf·b2bea5b359). 콘솔 오류 **0**. frame `__prof` total p95 4ms·draw 3.4ms·**hud 0.5ms**(≤16.7·렉 안 늘림). 바뀐 파일 셋뿐(git status 확인).
- **못 한 것(정직히)**: **② 톤·③ 아이콘 안 함** — 브리프 「하나라도 끝까지」로 ① 에 집중. 미흡:
  - (ㄱ) 구슬·벨트 **틀을 픽셀랩으로 굽지 않고 CSS**로 했다. 브리프는 픽셀랩 틀(`create_ui_asset`)을 밀었으나, 액체 구슬(결정적 D2 요소)은 CSS 로 충분히 읽히고 비동기 굽기의 위험·시간을 피했다 — **구슬 자체는 「코드 도형 때움」이 아니라 D2 그 요소**이고, 픽셀랩은 «청동 테두리 장식»에 얹는 다음 손질감(→ 다음 후보 ㉠).
  - (ㄴ) d2ui 에서 #tl 을 통째 숨겨 **피해배율(×dmgMul)·레벨 숫자**가 상시 HUD 에서 빠진다(D2 도 주 바에 없음·레벨은 C 창·필요하면 가운데 판에 작게 되살릴 것).
  - (ㄷ) 적 이름표(월드 플로터)가 가운데 벨트 판 위를 잠깐 스침 — 기존 라벨 계통이라 이 판 무접촉.
- **치워 둔 것**: V-285 미완 잔손질(`__CORPSEREAD`/`__ROTPOOL2`/`__BLOODGRAIN`/`__HERORIM`·커밋 안 됨)을 `git stash`(stash@{0})로 치워 V-287 을 깨끗한 베이스에서 시작했다. 되살리려면 `git stash pop`(단, D2_FEEL 이 잔손질을 접으라 했으니 되살릴지 판단 필요 — 특히 `__HERORIM`(쨍한 금테 낮춤)은 ② 톤과 겹치니 ② 판에서 함께 볼 것).

### ▶ 다음 후보(V-287 이 올림)
- **㉠ ② 톤을 어둡고 탁하게** — 바닥 밝기·채도를 내려 화면에서 제일 밝은 것이 «불»뿐이게. 단일 게이트 `floorBase`(hs/main.js·모든 바닥 타일이 지남)에서 `__TONE` 로 휘도·채도만 낮추고 `drawLight` 의 additive 불빛(warmGlow)은 안 건드림(불이 안 죽음). 재는 법: 화면 가장 밝은 20 화소가 불인지 수로.
- **㉡ ③ 아이콘·인벤 아이콘을 픽셀랩으로** — `ICON_PART`(hs/main.js:6372~) 가 여덟 부위를 `_px()` 사각형으로 그린다. `assets/ui/item/*.png` 로 굽고 `tex()` 로 실어 `iconBase` 의 `part(g)`(main.js:6431) 자리를 교체. `tmp/iconbake/` 스테이징·안 나은 건 옛것.
- **㉢ ① 구슬·벨트 픽셀랩 청동 틀** — 지금 CSS 이중 링을 픽셀랩 `create_ui_asset` 장식 테두리로 얹기(선택·있으면 더 D2).

## ✔ 끝난 판: V-284 — 17:30 감시: 지역이 «색 말고는» 안 갈린다 (㉮ __ZONEFLOOR · ㉯ __ZONESIG · ㉰ _pin_state) (2026-09-03)
17:30 감시가 넘긴 뿌리 둘 — ① 여섯 판 바닥 타일을 구웠는데 지역 wash 가 화면에서 그 색을 씻어 세 층이 다 따뜻한 갈색(hue 2~10°)으로 읽힘 · ② 25층이 «색만» 갈리고 지역 고유 장치가 없음. 만진 곳 `hs/main.js` 하나(그리기·AI·UI·fresh)·map.js/genFloor 무접촉·새 자 파일 없음(베이커 `tmp/hs_v284_cut.mjs`·look `tmp/hs_v284_look.mjs`·helper `tmp/_pin_state.mjs`).
- **㉮ `__ZONEFLOOR`** — 지역 wash 가 바닥 타일 색까지 통째로 씻던 것을 둘로 고침: (1) `zoneWash()` 가 wash 를 벽(wallTint)·소품(warm)·공기엔 그대로, **바닥(방∪복도 rect)에만 0.42배**(evenodd 로 도려냄). (2) `zoneFloorTint()` 가 지역마다 다른 바닥색을 **휘도 보존 "color" 블렌드(STR 0.7)**로 밀어 넣음 — 밝기 mean·텍스처 sd 는 그대로, 색상만. **재서 적음**(층3 crypt/층10 abyss/층17 blood, 같은 FLOOR_CAM·같은 조명): hue 27.3·29.4·19.6(간격 2.1/9.8/7.7°) → **92·37.4·11.6(간격 54.6/25.8/80.4° — 셋 다 ≥25°)** · 밝기 mean 59.4·64.2·59.1 → **59.4·60·59.1(보존)** · 밝기 sd 14.9·12.4·8.1 → **14.9·13.4·8.9(안 줄음)**. 짝 PXDIFF crypt 30.7%·abyss 17.8%·blood 55.7%. **컷 직접 열어 봄**: 묘지=찬 청회색 무덤돌(벽은 warm)·뼈무덤=금빛 뼈바닥·피회랑=검붉은 회랑 — 세 곳이 확연히 «다른 곳». `__ZONEFLOOR=false` → 옛(통짜 wash·바닥색 씻김).
- **㉯ `__ZONESIG`** — 지역마다 몸으로 겪는 장치 하나씩(있는 계통만 되씀·한 덩어리). `zoneSigInit(fresh)`·`stepZoneSig`·`drawZoneSig`·`wakeNearest`.
  - **묘지(1~5)**: 관 ~60%가 곁에 시체 하나 더(초반 소환 밑천). 컷 `zonesig_grave_{on,off}`(관 곁 시체 5개↔0개). ★미흡: 시체 스프라이트가 V-257 로 작아(두개골 c.h×0.12≈19px) **뼈 무늬 크립트 바닥에 묻혀** 한 장에선 티가 옅다 — 장치는 실동(5↔0 확인·이른 층에 소환 자원 실제로 늘어남). 코드로 «시체 렌더 크기»는 안 건드림(다른 지역 공유).
  - **뼈무덤(6~10)**: 뼈무더기(bones 소품)를 밟으면 부서지며(먼지 링+뼛조각) 반경 560 안 **잠든 무리를 깨움**(조심해 걷게). 뼈무더기가 보임=예고. 컷 `zonesig_bone_{on,off}`(짝 PXDIFF 74.5%·부스러짐 먼지+에워싼 무리).
  - **썩은굴(11~15)**: 방마다 독 웅덩이(어두운 진창+독빛 젖은 테) — 밟으면 걸음 0.56배(구르기는 그대로=벗어나게). 웅덩이가 보임=예고. 컷 `zonesig_rot_{on,off}`(짝 PXDIFF 30.5%·초록 바닥에 값으로 갈리는 웅덩이).
  - `__ZONESIG=false` → 셋 다 옛대로(장치·그리기·느림 다 꺼짐).
- **㉰ `tmp/_pin_state.mjs`** — V-283 ㉰ 의 「빈 장면 두 장」 재발 막이. 짧은 상태(예고·부스러짐 등)를 촬영 «내내» 16ms 마다 도로 못박는 `PIN(mutBody)`/`UNPIN` 과 화면 전체 지표 `screenPct`. 이 판 베이커가 뼈무덤 crack 을 이걸로 못박아 찍음. **★교훈: 짧은 상태 컷은 이 helper 로 못박고, 좁은 상자 수치는 화면 전체로 한 번 받쳐라.**
- 되돌림 지문: 셋 다 그리기/AI/UI/fresh(genFloor 무접촉) → F1/3/5/10/30 **byte-동일**(4341720539·cebe184b88·07968a97a3·e9e3aa0cbf·b2bea5b359 = OFF=ON=BASE).
- 회귀(있는 자로만): `hs_v207_walk` **벽밖 0%·오류 0**(WAKE 3000/820 — 웅덩이 느림이 벽뚫·오류 안 냄) · `hs_v219_foeshot` **frame p95 1.6ms(≤16.7)·에셋 100%·오류 0·쏜화살 381**. 콘솔 오류 **0**·`md5 -q tmp/hs_v284_*.png|sort|uniq -d` **비었음**·짝 PXDIFF 다 ≥3%.
- **못 한 것**: ㉯ 는 **셋 지역(묘지·뼈무덤·썩은굴)만** — 피회랑(돌진 가속)·심연(시야 좁힘)·성소(기둥 엄폐)는 안 함(다음 후보). 미흡: 묘지 시체 가독성(위 ★). 밸런스는 회귀선(foeshot)만 봤고 한 지역만 hp 두 배 깎이는지는 실플레이로 못 쟀다(장치가 가볍고 예고가 있어 위험 낮음).
...
### ▲ 17:00 감시 재검(컷을 직접 열어 봄) — ㉰ 는 «증거 없음»이었다, 다시 구워 통과. ㉱ 는 과장.
16:39 에 자기판정으로 닫았으나, 17:00 감시가 컷 넷을 열어 보니 **㉰ 의 증거가 한 장도 없었다.**
- **㉰ 컷 무효 → 재베이크 후 통과.** `hs_v283_telegraph_{before,after,late}.png` **세 장 모두 화면전체 붉은화소 2.24 / 2.26 / 2.24%** — 즉 예고 도형이 어느 장에도 **안 담겼다**. 까닭: 베이커가 `__chargeTellPose()`(tele=`CHARGE_TELE*0.6`=**0.27s**)를 부른 뒤 **700ms 를 기다리고** 첫 장을 찍어, 그때는 이미 예고가 꺼지고 돌진으로 넘어간 뒤였다. 보고에 적힌 「17.4%→4.3%」는 사람 **80×90 상자**(대부분 사람 몸·HUD) 안 수치라 도형과 무관했다. → `tmp/hs_v283b_cut.mjs` 로 **tele 를 촬영 내내 못박아**(`setInterval` 재고정) 다시 구움: `hs_v283b_telegraph_{before,after}_crop3.png` — **before 는 사람 몸을 가로지르는 납작한 단색 붉은 판, after 는 근원이 밝고 끝이 옅어지는 gradient 띠**로 사람 실루엣이 비친다. 사람상자 **17.97%→9.04%**·PXDIFF **13.17%**·오류 0. **㉰ 통과(눈으로 확인).**
- **㉱ 짝이 무효였고, 고친 뒤 보니 «과장».** 옛 `abyss_grain_{before,after}` 는 **서로 다른 자리**(한쪽에만 구덩이)라 짝이 못 됐다 — `__restart` 재시도 횟수가 달라 seed 1337 이어도 맵이 갈렸다. 같은 자리로 다시 구우니(`hs_v283b_abyss_{before,after}_crop3.png` · 카메라 `[4295,887,3305,511]` 양쪽 동일) **차이가 눈에 겨우 보인다**: in-game 밝기 sd **21.3→21.7**(+0.4). 타일 파일 자체는 확실히 올랐으나(sd **30.4→40.6**·mean 32.1 불변) **화면에서는 조명·floorMix 가 대부분을 씻어낸다.** 「판석 격자 또렷」은 **타일 파일 이야기지 화면 이야기가 아니다** — 위 ㉱ 줄의 그 표현을 이 문단이 정정한다.
- **덤으로 드러난 것:** in-game abyss 바닥이 V-282 가 의도한 **「검푸른 재」가 아니라 따뜻한 갈색**으로 읽힌다(타일 파일은 실제로 검푸른데 층 9~10 의 warm wash 가 덮음). V-282 ㉰ 의 「지역색」이 화면까지 못 온다는 뜻 — **아래 「다음 후보」에 올림.**
- **교훈(자에 박을 것):** 짧은 상태(예고·피격·경직)를 찍는 베이커는 **촬영 내내 그 상태를 못박아야** 한다. 안 그러면 «빈 장면 두 장»이 큰 PXDIFF 로 통과한다. 그리고 좁은 상자로 잰 수치는 **화면 전체 대조**로 한 번 받쳐야 한다.

### ▶ 다음 후보(17:00 감시가 올림)
- **㉠ 바닥 지역색이 화면까지 못 온다** — 층 9~10 abyss 가 warm wash 에 씻겨 crypt 와 값·색으로 잘 안 갈린다. 존 wash 를 바닥 타일에는 약하게 걸거나(벽·소품엔 그대로), 타일 지역색을 wash 를 이길 만큼 밀 것. **재기 전에 crypt/abyss/blood 를 같은 자리·같은 조명으로 나란히 찍어** 사람 눈으로 갈리는지부터 볼 것.
- **㉡ 베이커에 「상태 못박기」를 상시 규칙으로** — `__pinTele` 같은 재고정 helper 를 `tools/` 로 올리고, 짧은 상태 컷은 전부 그걸 거쳐 찍게.
...
- **㉮ `__PROPRING`** — 소품 발밑 «쨍한 순노랑 닫힌 고리»(RTS 선택 원)를 사람 발밑(V-281 `drawFootRing`)과 같은 결로 낮춤. **grep 로 확인 — 닫힌 밝은 고리를 그리는 소품은 제단(`drawAltar`) 하나뿐**(신전=위 광원·화로=warmGlow·상자=풀). 새 `drawPropPool(x,y,rr)`: 채도·밝기 낮춘 던전 호박빛 접지 풀(닫힌 테 없음)+안쪽 룬 점 넷 맥동으로 「쓸 수 있다」만 읽힘(밝기로 화로 안 이김). 컷 `propring_before`(=`__PROPRING=false` 쨍한 고리)·`propring_after`(+crop3) 직접 봄 — 고리 사라지고 석상이 먼저 읽힘. 짝 화소차 **29.08%**. `__PROPRING=false` → 옛(V-281) 닫힌 고리.
- **㉱ `__STUCKDIR`** — 벽에 꽂힌 화살이 «촉(붉은 미늘)이 방 쪽·깃이 벽에 파묻힘»으로 벽에서 튀어나온 꼴이던 것(V-281 은 자리 16px 만 당기고 방향은 그대로)을, 국소 +x=날아간(벽으로 파고든) 방향으로 바로잡음: **쇠촉(회색)은 벽 안(+x)·붉은 깃털 두 갈래는 방 쪽(-x)**. `rotate(atan2(dy,dx))` 라 상·하·좌·우 벽 네 방향 다 같은 국소 규칙. 컷 `stuckdir_before`(=`__STUCKDIR=false` 거꾸로)·`stuckdir_after`(+crop3) 직접 봄 — 촉 벽으로·깃 방으로·검은 바깥 걸침 0. 짝 화소차 **7.61%**(120px→104×46 로 좁혀 잼). `__STUCKDIR=false` → 옛(V-281) 뒤집힌 꼴.
- **㉯ `__FLOORMIXCAP` + 타일 평탄화** — **「둥근 밝은 얼룩이 격자로 반복」의 진짜 근원은 구운 타일 비네트가 아니라 `floorBase` 의 `__FLOORMIX` 얼룩**(방마다 이웃 타일 하나를 큰 타원 셋으로 겹침)이었다(리뷰의 「타일에 빛이 구워졌다」 진단은 근접). 어두운 바닥(abyss mean 32) 위에 밝은 이웃(bone mean 86)을 α≈0.5 로 얹으니 밝은 얼룩이 됐다. **cap**: 섞는 타일이 바닥보다 밝을 때만 알파를 `max(0.34, 1-(am-bm)/60)` 로 줄임(어두운 얼룩은 옛대로 — 통과한 다른 층 무접촉). + 타일 자체도 토로이달 flat-field 로 저역(비네트) 평탄화 — abyss blockSd(8px 블록 평균휘도 sd) **8.7→3.1**(절반 이하)·sd 37.8→30.4(≥25). 컷 `floor_abyss_after2` 직접 봄 — **밝은 격자 사라짐·바닥 고름**. `__FLOORMIXCAP=false` → 옛 알파.
- **㉰ 타일 지역색**(휘도 보존 채도내림+틴트) — abyss 를 차가운 남보라+재빛으로(sat 0.5·cold tint), blood 를 채도·밝기 낮춰 마른 검붉은 돌로(mean 35.3→**31.7**·sd **26.3**·「쨍한 벽돌빨강」 사라짐). abyss mean **32.1**·sd **30.4** — 둘 다 **sd≥25**. 컷 `floor_blood_after2` 직접 봄 — 마른 검붉음(옛 밝은 벽돌빨강과 확연). **★ 발견: FLOOR_TILES↔ZONES 는 분리** — abyss_tile 은 층 9~10 에만 쓰이고 그 층은 **뼈무덤(zone1) 따뜻한 wash(0.11)** 다(심연 zone4 는 층 21~25·sanctum_tile). 그래서 in-game abyss 는 순blue 가 아니라 **재빛 어두운 회색**(blue 가 warm wash 를 상쇄해 중화)으로 crypt 의 밝은 따뜻한 tan 과 **값·채도로 갈림**(존과도 조화). 되돌림 `assets/floor/_pre_v282/`.
- 되돌림 지문: ㉮㉱㉯cap 는 그리기·㉰ 는 에셋 PNG 교체 → **genFloor 무접촉** → F1/3/5/10/30 **byte-동일**(4341720539·cebe184b88·07968a97a3·e9e3aa0cbf·b2bea5b359).
- 회귀(있는 자로만): `hs_v207_walk` **벽밖 0%·오류 0**(WAKE 3000/820) · `hs_v219_foeshot` **frame p95 1.6ms·오류 0·에셋 100%·쏜화살 384**. 컷 직접 열어 봄·콘솔 오류 **0**·`md5 -q tmp/hs_v282_*.png|sort|uniq -d` **비었음**·짝 PXDIFF(propring 29.08%·stuckdir 7.61%) **≥3%**.
- **못 한 것**: 없음(㉮㉯㉰㉱ 다 함). 미흡: ㉰ abyss 는 쓰이는 층(9~10=뼈무덤 warm zone)이라 in-game 은 순blue 가 아니라 재빛 어두운 회색(설계상 존과 조화·값으로 갈림). ㉯ 는 리뷰가 「타일 비네트」로 봤으나 실인은 floorMix 얼룩 — 타일 평탄화와 floorMix cap **둘 다** 넣어 밝은 얼룩을 없앴다. 브라우저가 HTTP 로 타일을 캐시해 새 타일이 안 뜨던 것은 베이커에 `Network.setCacheDisabled` 를 넣어 고침.
