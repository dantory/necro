...
## ▶ 지금 도는 판: **V-252** — 적이 «수법대로» 달라 보이게 (착수 2026-09-02 16:0x)
브리프 `/Users/lbs/.openclaw/workspace/tmp/v252_prompt.md` · 로그 `/Users/lbs/.openclaw/workspace/tmp/necro_hs_v252.log`
감시가 살아 있는 캐릭터 여덟의 south.png 를 직접 보고 골랐다(`tmp/hs_v252_chars.png`). 낱장 품질이 아니라 **판에서 안 갈리는 것**이 문제였다:
① **폭탄병과 돌격병이 같은 `mob/brute` 몸**(main.js:1706-1707) — 수법이 정반대인 둘이 픽셀 하나 안 다르다 → 폭탄병 전용 몸을 굽는다.
② **넷이 다 붉다**(brute·shaman·fallen·boss) — 게임에서 h 59~82px 로 그려져 덩치 말고는 안 갈린다 → 갈래별 색조(`__MOBTINT`).
③ V-251 이 남긴 흠 — **계단이 우물처럼 보인다**(워커가 스스로 적어 놓고 못 고쳤다). 한 번 더 시도, 또 실패하면 접고 적는다.
