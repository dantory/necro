...
## ▶ 지금 도는 판: **V-237** — 적의 «수법»이 하나뿐이다 (잡몹 갈래 셋) · 겹치는 글판 · 장비줄 낀 칸 (띄움 2026-09-02 04:3x)
감시가 V-236 컷(`tmp/hs_v236_gem_buy.png` 등)을 **직접 열어 보고** 정한 세 항목.
① **잡몹 갈래 셋**(`__MOBKIND`) — 돌진꾼(예고선→돌진→벽에 박히면 빈틈) · 사수(`stepRanged` 정식 편입) ·
   **시체 도둑**(바닥 시체를 먹어 없앤다 — 이 게임의 자원을 뺏는 첫 적. 「서둘러 써야 한다」는 긴장).
   지금 잡몹은 `elite` 와 숨은 `__RANGED_MOB` 말고는 **전부 같은 수법**이고, 층 주인만 시전을 갖는다.
② **떠 있는 글판이 서로 덮고 화면 밖으로 잘린다**(`__NOTESTACK`) — 초록 구입글이 제단 판의 「최대 생명 +8%」·「피 140◈ B」를
   통째로 가린다. 집는 글(「완벽 루비」)은 왼쪽이 잘림 — V-236 의 `clampLabelX` 를 **제단 이름표에만** 걸었다.
③ **장비줄에서 낀 칸/빈 칸이 구분 안 됨**(`__GEARCOLOR`) — 일곱 칸이 같은 색. 낀 것은 레어도 색으로.
