...
## ✔ 끝난 판: V-290 — 「불을 D2 의 불로」 하나만 (㉠ 흰 코어를 주황으로 · ㉡ 방마다 잔불) (2026-09-03 21:30)
V-289 가 올린 후보 ①②를 한 축으로 팠다. 20:30 감시가 컷을 열어 잰 진단이 출발점 — **우리 「불」은 주황이 아니라 흰 얼룩이었다**(가장 밝은 0.5% 화소의 불채도 **9.3% vs D2 53.8%**·흰색비율 **83% vs 1%**). 채도 덮개(V-289 ㉮)가 불꽃 스프라이트를 회색으로 씻고 그 위에 additive warmGlow 가 겹쳐 코어가 255 백색으로 blow 된 것. 만진 곳 `hs/main.js` **하나**(그리기)·genFloor/map.js 무접촉.
- **㉠ `__FIRECOLOR` 주황 코어** — additive **다음에** 불자리마다 진한 주황(`FIRE_MUL=255,190,95`·hue 36°·채도 63%)을 **multiply 로 좁게** 얹는다(`fireCore`). R=255 를 지켜 휘도(불이 제일 밝다)를 보존하면서 G·B 만 곱해 내려 **흰 코어를 R>G>B 주황으로 되돌리고** blow 된 밝음도 같이 내린다. 반경은 불 크기만큼(사람 138·화로 88px) — 바닥·halo 밖은 안 건드림. additive 반경도 250→150(사람)·150→90(화로)으로 줄여 **D2 의 「작은 불」** 결로. `__FIRECOLOR=false` → V-289(흰 코어).
- **㉡ `__EMBERS` 방마다 잔불** — D2 는 작은 불이 방마다 흩어져 「불이 유일한 색」이 화면 대부분에 닿는데 우리는 화로가 셋뿐이라 안 닿았다. 방마다 2~3개를 **결정적 hash**(전역 `Math.random` 무접촉)로 흩고, 화로와 같은 결(warmGlow+fireCore)이라 덮개 뒤로 걸려 주황이 남는다. 화면 밖 방은 건너뛰고 프레임당 상한 `EMBER_CAP=20`. `__EMBERS=false` → 없음.
- **같은 자로 잼**(floor1·화로 든 자리·seed 1337·프레임 4장 평균, `tmp/hs_v290_fire.png`):
  |          | 불채도%(top0.5) | 흰색% | 화면채도% | 어둠% | 밝음% | 평균휘도 |
  |----------|------|------|------|------|------|------|
  | before(289) | 6.9 | 96.8 | 13.4 | 60.9 | 2.0 | 25.1 |
  | **after**| **34.5** | **37.2** | **15.1** | **67.2** | **1.3** | **21.3** |
  | D2 도면  | 55.4 | 3.7 | 11.2 | 58.4 | 0.8 | 22.2 |
  불채도 **6.9→34.5**(목표 ≥35·프레임 흔들림 32.3~38.7 로 문턱에 걸침) · 흰색 **96.8→37.2** · 밝음 2.0→**1.3** · 화면채도 15.1(목표 12~15 상단) · 어둠·평균휘도 다 D2 결. **불 화소만 따로 보면 채도 50.4 ≈ D2 55.4** — 불 자체는 확실히 D2 색이 됐고, 남은 흰색/밝음은 덮개 밖 UI(구슬·이름표) 몫이다.
- **컷 직접 열어 판정**(`tmp/hs_v290_fire.png` before/after/D2): before 는 화로가 **희끄무레한 얼룩**이라 어디가 불인지 안 보였는데, after 는 **주황 불빛이 화로마다 웅덩이로 고이고 해골 무리가 그 빛에 물든다** — D2 도면의 「검은 방 + 주황 불 초점」과 같은 결 ✓. 이름표·해골·구슬 UI 다 읽힘.
- 회귀: `tmp/_v276_fp.mjs` F1/3/5/10/30 **byte-동일**(잔불이 지문 무접촉) · `hs_v219_foeshot` **frame p95 3.1ms(≤16.7)·에셋 100%·오류 0**·화살 밝기 182.8≥129.9 · `hs_v207_walk` **벽밖 0%·오류 0**(WAKE 3000/820) · 콘솔 오류 **0**.
- **못 한 것**: (ㄱ) 불채도 34.5 는 목표 35 에 **0.5 모자라고 프레임마다 32~39 로 흔들린다**(불꽃 애니 위상) — 파레토가 끝난 자리라 더 밀면 화면채도가 15 를 넘는다. (ㄴ) 흰색 37.2%·밝음 1.3% 는 D2(3.7/0.8)보다 높은데, 남은 건 **덮개 밖 UI**(구슬·붉은 판·이름표)라 이 축에서는 못 내린다 — UI 를 손대야 하는데 병수님이 「UI 말고 분위기」라 했으므로 안 건드림.

### ▶ 다음 후보(V-290 이 올림)
- **㉠ 몬스터에 색을 조금 되돌리기** — V-289 가 올린 후보 ②(아직 안 함). 지금은 적·소환수까지 무채색인데 D2 는 몬스터에 색이 남는다(피·갑옷·마법 접미 몬스터).
- **㉡ 덮개 밖 UI 가 top 화소를 먹는 문제** — 자(불채도/흰색)의 분모에 UI 가 끼어 진단이 흐려진다. 자를 전장 rect 로 좁히면 더 정직해진다.

## ✔ 끝난 판: V-289 — 19:34 병수님 「하단 UI 보다 게임 전체적인 분위기가 다르다」 → 분위기 하나만(㉮ 채도 덮개 · ㉯ 국소 조명) (2026-09-03)
V-287(UI)은 병수님이 말한 게 아니었다 — UI 는 손 안 대고(구슬 잘림 마감도 안 함) **분위기 하나만** 팠다. 같은 자로 재니 **어둡기는 이미 같고(어둠<25 우리 53% vs D2 58%) «채도»가 3.2배**(우리 36% vs D2 11%)였다 — 「더 어둡게」가 아니라 「탁하게」. 만진 곳 `hs/main.js` **하나**(그리기)·map.js/genFloor 무접촉·새 자 파일 없음(베이커 `tmp/hs_v289_mood.mjs`·sweep `tmp/hs_v289_sweep.mjs`).
- **㉮ `__DESAT` 채도 덮개(본체)** — 전장을 다 그린 뒤 화면 전체를 회색 판 하나로 **한 번** 덮어(`globalCompositeOperation='saturation'`·fillRect 한 번이라 쌈) 채도만 내린다(휘도·색상 보존). `drawMood()`(drawWorld 끝·restore 앞). ★ **불빛(warmGlow)·시체 후광을 덮개 «뒤»로 옮겼다**(drawLight → 새 `drawFireGlow`) — 덮개보다 먼저 걸면 saturation 합성이 주황을 씻는다. 그래서 **불이 유일한 색으로 남는다**(D2 의 핵심). 재보니 saturation 합성은 alpha 에 선형이 아니라, `__DESAT=1`+불끄면 채도 정확히 0 → **잔여 채도는 전부 불빛 주황**이었다. 그래서 불 반경·세기를 줄여(`FIRE_STR` 0.45·사람 횃불 320→250px) 「방 전체 주황 워시」를 「둘레만」으로. `__DESAT` 은 **0.9**(1.0 아님) — 색조를 살짝 남겨 **㉰ 존 갈림을 지킴**(아래). `__DESAT=false`/`__FIREGLOW=false`/`__DESATMODE` 되돌림·시험 손잡이.
- **㉯ `__VIGN` 국소 조명** — 방 전체가 균일히 밝아 「밝은 실내」로 읽히던 것을, **한 번 구운 radial 스프라이트**(vignSprite·매 프레임 생성 금지)를 사람 자리에 얹어 내 둘레(반경 `VIGN_R=620`월드px·적 무리 하나 드는 정도)만 밝고 멀수록 어둠. 불 둘레는 drawFireGlow 가 되밝힘 → **「내 둘레와 불만 밝다」**(D2). `__VIGN=false` → 균일.
- **㉰ 존 갈림 유지** — V-284 가 벌린 hue 간격(54.6/25.8/80.4°)이 채도를 올렸을까 봤다. `__DESAT=1` 은 색조를 다 죽여 **F17(피회랑) 채도 16.7 ≈ F1(묘지) 13.8**(존이 안 갈림). `__DESAT=0.9` 로 낮추니 **F17 20.4 vs F1 13.3**(붉은 존이 여전히 더 짙음·눈으로 갈림) — **갈림 살리고 채도 목표 둘 다** 잡음. hue 간격 자체는 안 건드림.
- **같은 자로 잼**(floor1·화로 든 자리·seed 1337, `tmp/hs_v289_mood.png` 상단):
  |          | 채도% | 어둠<25% | 밝음>110% | 평균휘도 |
  |----------|------|---------|----------|--------|
  | before   | 46.8 | 53.0    | 1.7      | 28.7   |
  | **after**| **13.5** | **60.9** | 1.8  | **24.7** |
  | D2 도면  | 11.2 | 58.4    | 0.8      | 22.2   |
  채도 **46.8→13.5(목표 12~15·D2 11.2)** ✓ · 평균휘도 **28.7→24.7(D2 22.2)** ✓ · 어둠 53→60.9(**D2 58.4 수준**·vignette 만큼 오름).
- **컷 직접 열어 판정**(`tmp/hs_v289_mood.png` 위 우리·아래 D2): 인상이 «따뜻한 갈색 밝은 실내»→«차가운 회색 돌던전 + 주황 불»로 **뒤집힘 — 「같은 게임의 화면으로 보이기 시작한다」** ✓. 해골 무리·이름표(망령·사수·시체도둑)·구슬 UI 다 읽힘(가독 실패 없음). 밖은 검정(D2 결).
- 회귀: `tmp/_v276_fp.mjs` F1/3/5/10/30 **byte-동일**(genFloor 무접촉) · `hs_v219_foeshot` **frame p95 3.5ms(≤16.7·렉 안 늘림·덮개 +~1.9ms)·오류 0**·화살 밝기 182.8≥129.9(desat 뒤에도 읽힘) · `hs_v207_walk` **벽밖 0%·오류 0** · 콘솔 오류 **0**.
- **못 한 것/미흡**: (ㄱ) **밝음>110 1.7→1.8**(목표 <1·D2 0.8) — 밝은 화소는 겹친 불빛 코어(255 백색)라 더 줄이면 채도가 목표 밑으로 떨어진다(맞바꿈). 평균휘도·채도는 D2 에 닿았으니 이 한 지표만 남김. (ㄴ) 「불이 이긴다」 상위화소 잣대는 사람 횃불이 화로 70px 밖까지 번져 **과소집계**(25.5→19%)되나, 컷에선 불이 확연히 지배 — v288 잣대의 한계라 v289 는 나란히 컷으로 판정. (ㄷ) UI 구슬·벨트·붉은 위험띠는 DOM/브리프 지시대로 덮개 밖(D2 도 구슬은 선명).

### ▶ 다음 후보(V-289 가 올림)
- **㉠ 밝은 코어 줄이기** — 불빛 additive 가 겹쳐 백색(255)으로 blow → 밝음>110 이 D2 의 2배. 코어 상한(불투명도 캡)이나 채도 있는 주황을 지키며 흰 코어만 낮출 길(밝음<1 목표).
- **㉡ 지역별 채도 미세조정** — `__DESAT` 이 전역 0.9 라 밝은 존(성소·묘지)도 어두운 존(심연)도 같은 세기. 존 dark 처럼 존별 desat 로 D2 의 「갈려도 다 탁함」을 더 정밀히.
- **㉢ 불이 작고 많게(D2 결)** — D2 는 작은 불이 사방에 흩어짐. 우리는 화로가 큼직·드묾. 바닥 잔불/작은 화톳불을 흩어 「불이 유일한 색」을 더 살릴 것.
