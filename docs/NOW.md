...
## ▶ 지금 도는 판: V-265 — 지도에 «길»을 그린다 · 전체지도 대비 · 발사체 벽밖 0% (착수 2026-09-03 01:32)
브리프 `tmp/hs_v265_prompt.md` · 로그 `~/.openclaw/workspace/tmp/necro_hs_v265.log(+.done)` · 워커 PID 76176(상한 2700초 → 02:17 께).
감시가 V-264 컷을 직접 열어 보고 집은 흠 둘 + ROADMAP 맨 끝 열린 항목 하나:
- **① `__MAPPATH`** — `renderMapStatic` 이 `G.rooms` 만 그리고 **`G.corridors` 를 한 번도 안 그린다**. 방이 허공에 흩어져 계단까지 길이 안 읽힌다.
- **② `__BIGMAPDIM`** — 층 10 전체지도(`bigmap_deep`)가 뒤 화면이 비쳐 흐리다. 뒤판 어둡게·대비 올림(미니맵은 안 건드림).
- **③ `__SHOTWALL`** — ROADMAP **V-224**(발사체 벽밖 5~8.6%). 스텝 쪼개 터널링 차단 · 끝 조건 벽밖 0%.
