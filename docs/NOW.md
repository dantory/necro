...
## ▶ 지금 도는 판: V-266 — 죽음의 값(`__DEATHCOST`) · 시체 회수(`__CORPSERUN`) · V-265 지도 흠 하나 (띄움 2026-09-03 02:33)
브리프 `tmp/v266_brief.md` · 로그 `~/.openclaw/workspace/tmp/necro_hs_v266.log(.done)`. V-250(중단분 `7854bac`)을 이어받는다.
**이 칸을 읽는 워커는 «너 자신»이 그 판이다 — 물러서지 말 것.** 끝내면 이 칸을 「✔ 끝난 판」으로 고쳐 쓴다.
