...
## ▶ 지금 도는 판: **V-247** — 지역 여섯(구간마다 다른 곳·`__ZONE`) · V-246 컷 흠 다섯 (12:40 띄움·PID 11272)
브리프 `tmp/v247_prompt.md` · 로그 `~/.openclaw/workspace/tmp/necro_hs_v247.log(+.done)` · 착수 커밋 `91f004c`
V-246 컷 흠 다섯(12:30 감시가 눈으로): (a) 우리뼈 스프라이트가 벽 밖에 널림(선만 클립됨) (b) 주인 붉은 경고 «원»이 벽 넘음 (c) 번개 십자 점선도 벽 넘음 (d) 뼈 껍질 육각 테가 이름 글 관통 (e) 정예 이름 외곽선 없어 벽 무늬 위에서 안 읽힘.
