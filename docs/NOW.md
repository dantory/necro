...
## ▶ 지금 도는 판: **V-236 — 아이템 심화(소켓·보석 박기) · 제단 이름표 화면 밖 잘림**
띄움 2026-09-02 03:37 · 로그 `tmp/necro_hs_v236.log(.done)` · 착수 커밋 `072f648`.
감시가 V-235 컷(`hs_v235_altar_close.png` 등)을 직접 열어 보고 정했다 — 주운 물건에 **사람이 손대는 길이 없다**(굴린 대로 입거나 버릴 뿐).
소켓 1~2칸(낮은 레어도에 더 잘) + 보석 네 종×등급 셋(드랍·가방·박으면 못 뺌) + 제단에서 보석 구입 + 이름표 clamp.
되돌림 `__SOCKET`·`__GEM`·`__LABELCLAMP`.
