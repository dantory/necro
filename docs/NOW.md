...
## ▶ 지금 도는 판: V-255 — 무리가 «진형»으로 싸운다 (착수 2026-09-02 18:35)
감시가 V-254 컷(`tmp/hs_v254_order_{follow,attack,hold}.png`)을 직접 열어 골랐다. 브리프 `tmp/v255_prompt.md`.
① **무리 진형(`__FORMATION`)** — 갈래를 «보이게»는 갈랐는데(V-252~254) **움직임은 다 같다**. 사수는 사거리 지키며 물러서고·돌진꾼이 앞줄·자폭병은 돌아 들어와 터지고·도둑/망령은 시체로 샌다. 겹침 분리 힘을 **모든 적**에(왜 `__CROWDSPREAD` 가 방 밖 무리엔 안 먹는지부터).
② **긴 힌트 줄이 두 줄로 넘쳐 겹침** — `CONTROLS` 28개를 한 줄에 이어 붙여 화면 폭을 넘는다(V-254 의 덤). 한 근원은 지키되 긴 줄엔 자주 쓰는 것만.
③ **「여기 지켜」 깃발이 방 밖 벽 위** — 지켜 자리를 통행 칸으로 클램프.
④ **시체가 어두운 바닥에서 붉은 고리뿐** — 「시체가 자원」인데 주울 것이 안 읽힌다.
