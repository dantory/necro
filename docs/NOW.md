...
## ▶ 지금 도는 판: **V-240 — 군세 갈래(구울 `K` · 골렘의 결) · 군세 구성 HUD · 40층 이름표 도배/잘림** (착수 2026-09-02 06:35)
브리프 `tmp/v240_prompt.md` · 로그 `~/.openclaw/workspace/tmp/necro_hs_v240.log(.done)`. 왜: 적은 V-237 에서 갈래 셋으로 벌어졌는데 **내 편은 해골 하나**다(골렘은 같은 스프라이트의 「센 해골」·구울은 없음). 판정은 PLAN ③ — 어느 종도 70% 안 넘음.
