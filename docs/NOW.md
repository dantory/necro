...
## ▶ 지금 도는 판: **V-234 — 금을 쓸 곳(뼈 제단) · 손잡이 줄이 손잡이를 덮는다** (띄움 2026-09-02 02:0x)
V-233 컷 셋을 감시가 직접 열어 보고 낸 두 항목.
① **금이 죽은 자원** — `G.gold` 는 쌓이기만 하고 쓰는 자리가 **하나도 없다**. 층마다 **뼈 제단**
   하나(피/뼈/재 셋 중 굴림)를 상자와 같은 길로 놓고 **B** 로 산다. 새 에셋 없음(statue+금빛 룬 고리).
② **`#hint` 가 `#bl` 을 덮는다** — 컷 셋 전부에서 두 줄로 접혀 손잡이 칸을 가로질러 못 읽는다.
   짧은 한 줄 + **H** 로 전체 판 열기.
되돌림 `__ALTAR=false` · `__HINTFOLD=false`. 회귀는 있는 자로만. 컷 `tmp/hs_v234_{altar,buy,hint}.png`.
