...
## ▶ 도는 판: V-275 — 10:00 감시 흠 넷 + 저주받은 신전(`__CURSEDSHRINE`) (착수 2026-09-03 10:0x)
10:00 감시가 V-274 컷 **열둘**을 합성 시트 넷으로 직접 열어 판정: **대체로 통과**(②줌 before/after 확연 · ③제목 위로 · ④지도 불투명+범례 13항목 · ①발동 뒤 구멍 또렷 · ⑤신전 idle→used→HUD 흐름 확인). 흠 넷을 넘긴다. 브리프 `tmp/v275_brief.md`.
- **㉮ 축복 HUD 띠가 층 제목을 정면으로 덮는다** — `shrine_hud` 컷에서 붉은 「피의 신전 45s」 띠가 「뼈 무덤」 글자 위에 그대로 겹쳐 둘 다 못 읽는다. V-274 노트의 「~2.8s 살짝」이 아니라 **통째로 가림**. ③(제목 위로)과 띠(y74)가 같은 자리를 놓고 싸운다 → **한 근원 `topBandLayout()`** 으로 세로로 쌓는다(`__TOPSTACK`).
- **㉯ 신전이 랜드마크로 안 읽힌다** — `shrine_idle` 에서 신전은 방 오른쪽 **작고 흐릿한 붉은 얼룩**, 같은 방 화로보다 작고 어둡다. 크기·꼴로 눈에 띄게(밝기로 이기지 말 것·화로가 제일 밝음 유지)+갈래 넷 색이 몸에(`__SHRINELOOK`).
- **㉰ 바닥 꺼짐 armed 결이 1배율에서 아직 흐릿** — 3배 크롭에선 또렷하나 밝은 이끼 바닥 위 전체 컷에선 주변 타일과 거의 안 갈림. 발동 뒤(구멍)는 또렷하니 **armed 만** 이중선(어두운 금+밝은 하이라이트)으로.
- **㉱ 컷 두 장이 byte-동일** — `hs_v274_map_legend4.png` == `hs_v274_map_opaque.png`(md5 `e7787f4d…`). 한 장을 두 이름으로 냈다. 이번 판부터 `md5 -q tmp/hs_v275_*.png | sort | uniq -d` 가 비어야 커밋.
- **⑤ 저주받은 신전 `__CURSEDSHRINE`(새 컨텐츠)** — V-274 신전은 **공짜**라 보이면 무조건 쓴다 = 고를 것이 없다. 층3부터·층당 0~1·검붉은 신전·**E** 로 축복 하나 + **대가 하나**를 같이(피의 계약 피해×1.8/받는 피해×1.45 · 뼈 자리+8/최대체력−25% · 굶주린 드랍×2.0/소환수체력−40% · 성급한 속도×1.45/시전간격×1.35). genFloor 맨 끝·산술 PRNG·fp 밖 → 지문 byte-동일. HUD 는 ㉮ 의 topBandLayout 으로 축복(붉음)+대가(잿빛) 함께.
